...
class SongEncoder(nn.Module):
    def __init__(self, vocab_size, text_emb_dim, num_filters, filter_size, output_dim):
        super(SongEncoder, self).__init__()
        self.embedding = nn.Embedding(vocab_size, text_emb_dim, padding_idx=0)
        self.conv = nn.Conv1d(in_channels=text_emb_dim, out_channels=num_filters, kernel_size=filter_size)
...
class RecommendationModel(nn.Module):
    def __init__(self, num_users, user_emb_dim, vocab_size, text_emb_dim, num_filters, filter_size, dropout_p=0.5):
        super(RecommendationModel, self).__init__()
        self.user_embedding = nn.Embedding(num_users, user_emb_dim)
        self.song_encoder = SongEncoder(vocab_size, text_emb_dim, num_filters, filter_size, user_emb_dim)
    ...
    def forward(self, user_idx, song_tokens, numeric_feature):
        ...
        combined = torch.cat([user_emb, song_emb, numeric_emb], dim=1)
...
def main():
    ...
    model = RecommendationModel(len(user2idx), user_emb_dim, len(vocab), text_emb_dim, num_filters, filter_size, dropout_p)
    ...
    print("\nStarting training for 20 epochs (main model)...")
    ...
    detailed_evaluation_test(model, test_eval, user_records, user2idx, song_meta_cache, vocab, max_len, device, score_mean, score_std, output_path="user_details_score_normalized.txt")
    ...
    for epoch in range(1, mf_epochs + 1):
        ...
        for batch in mf_loader:
            ...
            loss.backward()
            mf_optimizer.step()
            total_loss += loss.item() * u_idxs.size(0)
